...
class PDF(FPDF):
    def create_table(self, table_data, title='', data_size=10, title_size=12, align_data='L', align_header='C',
                     cell_width='0', x_start='x_default', emphasize_data=[], emphasize_style=None,
        ...
        if x_start == 'C':
            ...
            center_table = margin_width / 3  # only want width of left margin not both
        ...
        # add title
        if title != '':
            self.multi_cell(0, line_height, title, border=0, align='j', ln=1, max_line_height=self.font_size)
            self.ln(line_height * 0)  # высота между таблицей и заголовком, регулируется умножением на значение
        ...
        if not isinstance(col_width, list):
            if x_start:
                self.set_x(x_start)
            for datum in header:
                self.multi_cell(col_width, line_height, datum, border=0, align=align_header, ln=1,
                                max_line_height=self.font_size)
                x_right = self.get_x()
            self.ln(line_height)  # ничего
            y2 = self.get_y()
            ...
            for i in range(len(header)):
                ...
                self.multi_cell(col_width[i], line_height, datum, border=0, align=align_header, ln=3,
                                max_line_height=self.font_size)
                x_right = self.get_x()
            self.ln(line_height*1.5)  # расстояние от заголовка до значений таблицы, нижний край head
            y2 = self.get_y()
            self.line(x_left, y1, x_right, y1) # верхняя линия заголовка
            self.line(x_left, y2, x_right, y2)
            ...
            for i in range(len(data)):
                ...
                for i in range(len(row)):
                    ...
                    if datum in emphasize_data:
                        ...
                        self.multi_cell(adjusted_col_width, line_height, datum, border=0, align=align_data, ln=3,
                                        max_line_height=self.font_size)  # ln = 3 - move cursor to right with same vertical offset # this uses an object named self
                self.ln(line_height)  # расстояние между строками таблицы
        y3 = self.get_y()
